...
public class TopologyInstance {
    ...
    /**
     * Returns the id of the destination switch that has the lowest cost route for the source switch.
     * @param srcId
     * @param dstIds
     * @return
     */
    protected Long getLowestCostDst(long srcId, List<Long> dstIds) {
    	
    	if (destinationRootedTrees == null) {
    		return null;
    	}
        
        List<BroadcastTree> broadcastTrees = new ArrayList<BroadcastTree>();
        
        for(Long dstId : dstIds) {
        	BroadcastTree broadcastTree = destinationRootedTrees.get(dstId);
        	if (broadcastTree != null) {
        		broadcastTrees.add(broadcastTree);
        	}
        }
        
        if(broadcastTrees.size() == 0) {
        	return null;
        }
    	
    	int lowestCost = MAX_PATH_WEIGHT;
    	Long dstIdChoosen = null;
    	
    	for(Long dstId : dstIds) {
    		int cost =  destinationRootedTrees.get(dstId).getCost(dstId);
    ...
    /**
     * Overloaded method done to the condition that there are several possible destinations. 
     * The chosen destination will be the lowest cost.
     * @param srcId
     * @param dstIds
     * @param cookie
     * @return
     */
    protected Route getRoute(long srcId, List<Long> dstIds) {
    	Long dstId = getLowestCostDst(srcId, dstIds);
